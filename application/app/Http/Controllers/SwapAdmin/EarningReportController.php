...
class EarningReportController extends Controller
{
    ...
    public function actionCraditDebit(Request $request){
        try{
            ...
            else if($request->wallet == 'DMC Wallet'){
                $this->addwalletlog($member->id, $request->action, 0, $request->description, $request->amount, $coin_rate, formatdecimal($request->amount/$coin_rate, 8), $created_at); 
            }
