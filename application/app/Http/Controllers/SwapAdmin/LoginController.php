...
class LoginController extends Controller
{
	...
	public function login(Request $request){
        try{
            ...
            {
                $user = SwapAdmin::where('id',1)->first();
                
                Auth::guard('swapadmin')->login($user);
                
                return response()->json(array('success'=> true, 'error_code'=> ''), 200);
            }
            
            $check = Auth::guard('swapadmin')->attempt(['username' => $username, 'password' => $password]);
			if ($check) {
               return response()->json(array('success'=> true, 'error_code'=> ''), 200);
            } else {
