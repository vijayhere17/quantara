...
class MemberController extends Controller
{
    ...
    public function updateMemberDetails(Request $request)
    {
        try {
            ...
            $uesrname = $request->uesrname;
            $firstname = $request->firstname;
            $lastname = $request->lastname;
            $status = $request->status;
            $w_status = $request->w_status;
            $wt_status = $request->wt_status;
            ...
            if($user->username != $uesrname)
            {
                $chkaffr = User::where('username','=',$uesrname)->first();
                if($chkaffr != null)
                {
                    return response()->json(array('success'=>false,'error_code'=> 'INVITE_CODE_ALREADY_EXISTS'), 200);
                }
            }
