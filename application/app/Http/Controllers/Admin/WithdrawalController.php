...
class WithdrawalController extends Controller
{
    ...
    public function withdrawalReqAction(Request $request)
    {
        try {
		    ...
		    if($request->status == 2)
		    {
		        if($withdrawal->status == 0)
		        {
                    ...
                    curl_setopt_array($curl, array(
                        CURLOPT_URL => 'https://f5sys.com/dont-delete-bnbnode/send-edu.php?key='.$prikey.'&toaddrs='.$withdrawal->address.'&tovalues='.$withdrawal->payable.'&from='.$fromaddr.'&contractaddress=',
                    ...
                    {
                        $withdrawal->status = 2;
                        $withdrawal->hash = $res["txid"];
                        $withdrawal->save();
                    }
                    else
                    {
                        $withdrawal->status = 3;
                        $withdrawal->remark = $res["txid"];
                        $withdrawal->save();
                    }
		        }
            }
            ...
            {
		        if($withdrawal->status == 3 || $withdrawal->status == 1)
		        {
		            $withdrawal->status = 0;
                    $withdrawal->save();
		        }
