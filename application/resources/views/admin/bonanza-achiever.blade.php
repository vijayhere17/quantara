			...
			<div class="panel-body">
				<div name="form1" class="form-horizontal form-groups-bordered validate" id="form1" role="form">
				    <div class="form-group">
						<label for="field-1" class="col-sm-3 control-label">Select Bonanza</label>
						<div class="col-sm-5">
						    <select class="form-control" id="bonanza_id" onchange="oTable.draw();">
						        <option value="0">Select</option>
						        @foreach($allbonanza as $bonanza)
						        <option value="{{ $bonanza->id }}">{{ $bonanza->left_dmc }} : {{ $bonanza->right_dmc }} ({{ $bonanza->reward }})</option>
						        @endforeach
						    </select>
						</div>
					</div>
...
@section('jscontent')
	...
	<script src="{{ URL::asset('assets/admin/js/custom/bonanza-achiever.js') }}"></script>
@endsection
