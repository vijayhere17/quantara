...
function initiallize() {
    ...
    $('#tblData').on('click', '.edit-btn', function() {
        ...
        // Convert row to editable inputs with mobile-friendly styling
        row.find('td:not(:last-child)').each(function(index) {
            var currentValue = $(this).text();
            var columnName = oTable.column(index).dataSrc();
            $(this).html('<input type="text" class="edit-input form-control" style="width: 100%; box-sizing: border-box; padding: 5px;" value="' + currentValue + '" data-column="' + columnName + '">');
        });
    ...
    $('#newPackageBtn').on('click', function() {
        $('#addRow').show().find('input.new-input').css('display', 'block'); // Ensure all inputs are visible
